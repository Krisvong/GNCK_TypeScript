...
const db_1 = require("./db"); // Import the database connection pool from db.js
// Set up middleware
...
app.post("/todos", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const existingTodo = yield db_1.pool.query("SELECT * FROM todo WHERE description = $1 AND due_date = $2", [description, due_date]);
        if (existingTodo.rows.length > 0) {
            // If the todo already exists, update the existing todo by ID with the new description
            const { todo_id } = existingTodo.rows[0];
            const updateTodo = yield db_1.pool.query("UPDATE todo SET description = $1 WHERE todo_id = $2", [description, todo_id]);
            console.log("Todo updated:", updateTodo.rows[0]);
            res.json(updateTodo.rows[0]);
        }
        else {
            // If the todo doesn't exist, insert it into the database
            const newTodo = yield db_1.pool.query("INSERT INTO todo (description, due_date) VALUES($1, $2) RETURNING *", [description, due_date]);
...
app.post("/completed_todos", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { description, due_date } = req.body;
        // Insert the completed todo into the database
        const newCompletedTodo = yield db_1.pool.query("INSERT INTO completed_todo (description, due_date) VALUES ($1, $2) RETURNING *", [description, due_date]);
        console.log("New completed todo:", newCompletedTodo.rows[0]);
        // Delete the corresponding todo from the todo table
        const { todo_id } = req.body;
        const deleteTodo = yield db_1.pool.query("DELETE FROM todo WHERE todo_id = $1 RETURNING *", [todo_id]);
...
app.get("/completed_todos", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const allCompletedTodos = yield db_1.pool.query("SELECT * FROM completed_todo");
...
app.get("/completed_todos/:id", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const completedTodo = yield db_1.pool.query("SELECT * FROM completed_todo WHERE completed_todo_id = $1", [id]);
...
app.put("/todos/:id", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (description && due_date) {
            const updateTodo = yield db_1.pool.query("UPDATE todo SET description = $1, due_date = $2 WHERE todo_id = $3", [description, due_date, id]);
        ...
        // Check if only due_date is provided
            ...
            const updateTodo = yield db_1.pool.query("UPDATE todo SET due_date = $1 WHERE todo_id = $2", [due_date, id]);
...
app.delete("/todos/:id", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const deleteTodo = yield db_1.pool.query("DELETE FROM todo WHERE todo_id = $1", [
            id,
...
app.delete("/completed_todos/:id", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        // Delete the completed todo from the completed_todo table
        const deletedTodo = yield db_1.pool.query("DELETE FROM completed_todo WHERE completed_todo_id = $1 RETURNING *", [id]);
